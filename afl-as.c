...
static void add_instrumentation(void) {
  ...
  source_line = fopen("random-loc.txt","w");
  ...
  while (fgets(line, MAX_LINE, inf)) {
    ...
    /* In some cases, we want to defer writing the instrumentation trampoline
       ...
       the trampoline now. */
   if(strstr(line, ".loc")){
	//fprintf(source_line,"found");
	strncpy(loc,line,80); //store 
	//fprintf(source_line,"%s",line);
      }

    if (!pass_thru && !skip_intel && !skip_app && !skip_csect && instr_ok &&
        instrument_next && line[0] == '\t' && isalpha(line[1])) {
	random_value = R(MAP_SIZE);
      fprintf(outf, use_64bit ? trampoline_fmt_64 : trampoline_fmt_32,
              random_value);
  	if(flag!=0){
		fprintf(source_line,"%u\n",random_value);
		fprintf(source_line,"%s",prev);
	}
	else{
		fprintf(source_line,"%u\n",random_value);
		fprintf(source_line,"%s",loc);
		flag = 1;
	}
      instrument_next = 0;
      ins_lines++;
	...
	if(instrument_next==1){
		strncpy(prev,loc,80); //store 
    	 }
