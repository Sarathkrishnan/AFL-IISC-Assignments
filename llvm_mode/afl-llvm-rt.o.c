...
// Can make the declaration for New Memory here

/*
  static void __afl_map_c_shm(void) {

  u8 *id_c_str = getenv(C_SHM_ENV_VAR);

  if (id_c_str) {

    u32 shm_id = atoi(id_str);

    __afl_area_c_ptr = shmat(shm_id, NULL, 0);

    if (__afl_area_c_ptr == (void *)-1) _exit(1);

    __afl_area_c_ptr[0] = 1;
