...
static void setup_c_shm(void) {

  u8* shm_c_str;

  shm_c_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_c_id < 0) PFATAL("shmget() failed");

  atexit(remove_shm);

  shm_c_str = alloc_printf("%d", shm_c_id);

  setenv(SHM_C_ENV_VAR, shm_c_str, 1);
